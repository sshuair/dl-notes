...
"""
使用datasets.ImageFolder方式训练cifar
"""
...
trainloader = DataLoader(
    dataset=ImageFolder(root='../data/cifar/train_class', transform=transforms.Compose([transforms.ToTensor(),
...
class Net2(nn.Module):
    ...
    def forward(self, x):
        ...
        # 全连接层
        x = x.view(-1,
                   self.num_flat_features(x))  # the size -1 is inferred from other dimensions，由第二个参与自动计算第一位的参数，避免手动计算
        # 第一层全连接层
        x = F.relu(self.fc1(x))
        # 第二层全连接层
        x = F.relu(self.fc2(x))
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(16 * 5 * 5, 100)  # TODO: 生成方法，不要手动的计算这个数字
        self.fc2 = nn.Linear(100, 50)
        self.fc3 = nn.Linear(50, 10)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trainloader):
        ...
        loss = criterion(outputs, target)  # TODO:cretion 为何在这里直接用nn.CrossEntropyLoss不可以？
        loss.backward()
        optimizer.step()
